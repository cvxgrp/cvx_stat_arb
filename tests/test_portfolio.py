...
def test_build_with_weights(prices):
    portfolio = build_portfolio(prices, stocks=pd.DataFrame(columns=["A"], index=[pd.Timestamp("2013-01-01"),
                                                                                     pd.Timestamp("2013-01-02")],
                                                               data=2.0))
    pd.testing.assert_series_equal(portfolio.equity.sum(axis=1), pd.Series(index=[pd.Timestamp("2013-01-01"), pd.Timestamp("2013-01-02")], data=[3347.56, 3373.80]))
...
def test_multiply(prices):
    ...
    pd.testing.assert_series_equal(portfolio.equity.sum(axis=1), 2.0*pd.Series(index=[pd.Timestamp("2013-01-01"), pd.Timestamp("2013-01-02")], data=[3347.56, 3373.80]))
