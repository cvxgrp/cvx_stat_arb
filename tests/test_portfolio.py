...
def test_build_with_weights(prices):
    portfolio = EquityPortfolio(prices, stocks=pd.DataFrame(columns=["A"], index=[pd.Timestamp("2013-01-01"), pd.Timestamp("2013-01-02")], data=2.0))

    equity = portfolio.equity.sum(axis=1).dropna()\
        .loc[[pd.Timestamp("2013-01-01"), pd.Timestamp("2013-01-02")]]
                                                               
    pd.testing.assert_series_equal(equity, pd.Series(index=[pd.Timestamp("2013-01-01"), pd.Timestamp("2013-01-02")], data=[3347.56, 3373.80]))
...
def test_multiply(prices):
    ...
    pd.testing.assert_series_equal(equity, 2.0*pd.Series(index=[pd.Timestamp("2013-01-01"), pd.Timestamp("2013-01-02")], data=[3347.56, 3373.80]))
...
def test_add(prices, resource_dir):
    ...
    index = pd.DatetimeIndex([pd.Timestamp("2013-01-01"), pd.Timestamp("2013-01-02"), pd.Timestamp("2013-01-03"), pd.Timestamp("2013-01-04")])
    pd.testing.assert_frame_equal(www, port_add.stocks[www.columns].loc[index], check_freq=False)
