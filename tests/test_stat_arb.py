...
def test_stat_arb_constructor(stat_arb_group, resource_dir):
    ...
    assets_test = (
        pd.read_csv(resource_dir / "stat_arb_assets.csv", index_col=0)
        .squeeze()
        .astype(float)
    )
...
def test_stat_arb_trading(
    stat_arb_group, prices_val, prices_test, prices_train_val, resource_dir
):
    ### Test simple linear trading strategy
    ...
    holdings_test = pd.read_csv(
        resource_dir / "holdings.csv", index_col=0, header=0, parse_dates=True
    )
    ...
    daily_profit_test = pd.read_csv(
        resource_dir / "daily_profit.csv", index_col=0, header=0, parse_dates=True
    ).squeeze()
    total_profit_test = pd.read_csv(
        resource_dir / "total_profit.csv", index_col=0, header=0, parse_dates=True
    ).squeeze()
    sr_profit_test = pd.read_csv(
        resource_dir / "sr_profit.csv", index_col=0, header=0, parse_dates=True
    ).squeeze()
