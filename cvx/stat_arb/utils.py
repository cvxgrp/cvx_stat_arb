...
def plot_all_portfolios(stat_arbs, data_train, data_val, data_test):
    ...
    plt.axvline(data_val.index[0], color='k',\
                label="train->val", linewidth=1)
    plt.axvline(data_test.index[0], color='k',\
        label="val->test", linewidth=1);

    plt.plot([data_train.index[0], data_test.index[-1]], [1,1], "k--",\
        label="constraint", linewidth=1)
    plt.plot([data_train.index[0], data_test.index[-1]], [-1,-1], "k--",\
            linewidth=1);
...
def determine_n_plots(candidate_stat_arbs, data_val, cutoff):
    ...
    for i, stat_arb in enumerate(candidate_stat_arbs):
        (p, s, mu, all_assets, holdings) = stat_arb.p, stat_arb.s, stat_arb.mu,\
             stat_arb.all_assets, stat_arb.holdings
...
def fill_ax(ax, data_train, data_val, data_test, s, mu, c):
    ...
    ax.axvline(data_val.index[0], color='k',\
            label="train->val", linewidth=10)
    ax.axvline(data_test.index[0], color='k',\
        label="val->test", linewidth=10)

    ax.plot([data_train.index[0], data_test.index[-1]], [1,1], "k--",\
        label="constraint", linewidth=10)
    ax.plot([data_train.index[0], data_test.index[-1]], [-1,-1], "k--",\
         linewidth=10)
